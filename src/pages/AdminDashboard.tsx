...
import BadmintonDeposit, { type BadmintonTeamSummary, type MatchCockSummary } from './BadmintonDeposit'
import WasitBadmintonSkor from './WasitBadmintonSkor'
...
export default function AdminDashboard() {
    ...
    const [matchSummaries, setMatchSummaries] = useState<MatchCockSummary[]>([])
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const [resStats, resRegs, resBadminton] = await Promise.all([
                ...
                fetch(`${apiBase}/api/admin/registrations`, { headers }).catch(() => null),
            ...
            if (resBadminton && resBadminton.ok) {
                ...
                setMatchSummaries(bData.matches || [])
            }
        } catch (err) {
    ...
    const handleUpdateStatus = async (newStatus: 'verified' | 'rejected') => {
        ...
        try {
            ...
            const res = await fetch(
                `${apiBase}/api/admin/registrations/${selectedRow._lomba}/${selectedRow.id}`,
                {
                    method: 'PATCH',
                    headers: {
                ...
                <div className="min-h-screen bg-slate-950 text-slate-100">
                            ...
                            {section === 'overview' && (
                                <>
                                    <div className="mb-6">
                                        ...
                                        <p className="text-sm text-slate-400 mt-1">Total pendaftaran seluruh cabang lomba Youth Fun Day 2026</p>
                                    </div>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
                                        ...
                                        >
                                            <div>
                                                ...
                                                <p className="text-[11px] text-slate-400 mt-0.5">Layar 1 — Input &amp; undian bagan</p>
                                            </div>
                                        ...
                                        >
                                            <div>
                                                ...
                                                <p className="text-[11px] text-slate-400 mt-0.5">Layar 2 — Bagan sinkron proyektor</p>
                                            </div>
                                    ...
                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                                        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5">
                                            ...
                                            <p className="font-display text-3xl font-bold text-amber-400 mt-1">{stats?.overall.total_pendaftar ?? '-'}</p>
                                        </div>
                                        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5">
                                            <p className="text-xs text-slate-400">Total Slot</p>
                                            <p className="font-display text-3xl font-bold text-emerald-400 mt-1">{stats?.overall.total_slot ?? '-'}</p>
                                        </div>
                                        {Object.entries(stats?.by_lomba ?? {}).map(([lomba, val]) => (
                                            <div key={lomba} className="bg-slate-900 border border-slate-800 rounded-2xl p-5">
                                                <p className="text-xs text-slate-400 capitalize">{lomba}</p>
                                                <p className="font-display text-3xl font-bold text-violet-400 mt-1">{val.pendaftar}</p>
                                            </div>
                                        ))}
                                    </div>
                                    ...
                                    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6">
                                        ...
                                        {fetching ? (
                                            <p className="text-slate-500 text-sm py-6 text-center">Memuat data...</p>
                                        ) : (
                                            <RegistrationTable
                                ...
                                <>
                                    ...
                                    {lombaSection === 'badminton' && (
                                        <div className="flex items-center gap-3 mb-6">
                                            <span className="text-xs text-slate-400 font-semibold uppercase">Filter Kategori:</span>
                                            <select
                                                value={badmintonSubFilter}
                                                onChange={(e) => setBadmintonSubFilter(e.target.value)}
                                                className="bg-slate-900 border border-slate-700 text-slate-200 text-xs rounded-lg px-3 py-2 outline-none focus:border-amber-400"
                                            >
                                    ...
                                    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6">
                                        {fetching ? (
                                            <p className="text-slate-500 text-sm py-12 text-center">Memuat data...</p>
                                        ) : filteredRows.length === 0 ? (
                                            <p className="text-slate-500 text-sm py-12 text-center">Belum ada pendaftar di kategori ini.</p>
                                        ) : (
                                            <RegistrationTable
                                                rows={filteredRows}
                            ...
                            {section === 'keuangan' && (
                                <>
                                    <div className="mb-6">
                                        ...
                                        <p className="text-sm text-slate-400 mt-1">Rekonsiliasi estimasi pemasukan berdasarkan status verifikasi pembayaran</p>
                                    </div>

                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                                        <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-2xl p-5">
                                            <p className="text-xs text-emerald-400 uppercase font-semibold">Pemasukan Terverifikasi</p>
                                            <p className="font-display text-3xl font-bold text-emerald-400 mt-1">{formatRupiah(totalVerifiedRevenue)}</p>
                                        </div>
                                        <div className="bg-amber-500/10 border border-amber-500/20 rounded-2xl p-5">
                                            <p className="text-xs text-amber-400 uppercase font-semibold">Menunggu Verifikasi</p>
                                            <p className="font-display text-3xl font-bold text-amber-400 mt-1">{formatRupiah(totalPendingRevenue)}</p>
                                        </div>
                                    </div>
                                                ...
                                                <tbody className="divide-y divide-slate-800/60">
                                                    {financeSummary.map((f) => (
                                                        <tr key={f.lomba}>
                                                            ...
                                                            <td className="py-3 px-3 text-slate-400">{formatRupiah(f.harga)}/{f.satuan}</td>
                                                            <td className="py-3 px-3 text-emerald-400">{f.verifiedUnit} {f.satuan}</td>
                                                            <td className="py-3 px-3 text-amber-400">{f.pendingUnit} {f.satuan}</td>
                                                            <td className="py-3 px-3 font-semibold text-emerald-400">{formatRupiah(f.verifiedRevenue)}</td>
                                                            <td className="py-3 px-3 text-amber-400/80">{formatRupiah(f.pendingRevenue)}</td>
                                                        </tr>
                                                    ))}
                                                </tbody>
                                            </table>
                                    ...
                                    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6">
                                        ...
                                        <p className="text-xs text-slate-400 mb-4">Cocokkan tiap bukti transfer dengan mutasi rekening sebelum verifikasi.</p>
                                        <div className="overflow-x-auto">
                                            <table className="w-full text-left text-xs border-collapse">
                                                <thead>
                                                    <tr className="border-b border-slate-800 text-slate-400 uppercase">
                                                    ...
                                                    {financeRows.filter((r) => r.bukti_bayar_url).map((r) => (
                                                        <tr key={r.id} className="hover:bg-slate-800/30">
                                                            ...
                                                            <td className="py-3 px-3 text-center">
                                                                <a href={r.bukti_bayar_url} target="_blank" rel="noopener noreferrer" className="text-amber-400 hover:underline font-semibold">
                                                                    Lihat ↗
                                                                </a>
                                                            </td>
                                                        </tr>
                    ...
                    {selectedRow && (
                        <div className="fixed inset-0 z-[60] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
                            <div className="bg-slate-900 border border-slate-800 rounded-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto p-6 sm:p-8 space-y-6">
                                <div className="flex justify-between items-start border-b border-slate-800 pb-4">
                                    <div>
                                        <div className="flex items-center gap-2 mb-1">
                                            <span className="uppercase text-[10px] font-bold px-2 py-0.5 rounded bg-amber-400/10 text-amber-400 border border-amber-400/20">
                                                Lomba: {selectedRow._lomba}
                                            </span>
                                            {selectedRow.kategori && (
                                                <span className="uppercase text-[10px] font-bold px-2 py-0.5 rounded bg-emerald-400/10 text-emerald-400 border border-emerald-400/20">
                                    ...
                                    <button onClick={() => setSelectedRow(null)} className="text-slate-400 hover:text-white text-xl font-bold">✕</button>
                                </div>

                                <div className="space-y-4">
                                    <h3 className="font-display text-xs font-bold text-amber-400 uppercase tracking-wider">Rincian Atribut Lengkap</h3>
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 bg-slate-950 p-4 rounded-xl border border-slate-800 text-xs">
                                        {Object.entries(selectedRow)
                                            ...
                                            .map(([key, val]) => {
                                                ...
                                                return (
                                                    <div key={key} className={`p-2.5 rounded-lg border border-slate-800/80 bg-slate-900/40 ${isFile ? 'col-span-1 sm:col-span-2' : ''}`}>
                                                        <p className="text-[10px] text-slate-500 uppercase font-bold mb-1">{formatLabel(key)}</p>
                                                        {isFile ? (
                                                            <div className="flex flex-col gap-2 mt-1">
                                                                {val.match(/\.(jpeg|jpg|gif|png|webp)/i) && (
                                                                    <img
                                                                        src={val}
                                                                        alt={key}
                                                                        className="w-full h-40 object-cover rounded-lg cursor-pointer hover:opacity-90 border border-slate-800"
                                                                        onClick={() => window.open(val, '_blank')}
                                                                    />
                                                                )}
                                                                <a href={val} target="_blank" rel="noopener noreferrer" className="text-xs text-amber-400 hover:underline font-semibold flex items-center gap-1">
                                                                    📎 Buka File ({formatLabel(key)}) ↗
                                                                </a>
                                                            </div>
                                                        ) : (
                                                            <p className="font-semibold text-slate-200 break-words">{val !== null && val !== undefined && val !== '' ? String(val) : '-'}</p>
                                                        )}
                                                    </div>
                                                )
                                            })}
                                ...
                                <div className="border-t border-slate-800 pt-4 space-y-4">
                                    <label className="block">
                                        <span className="text-xs text-slate-400 mb-1 block">Catatan Verifikasi Admin</span>
                                        <input
                                            type="text"
                                            value={adminNoteInput}
                                            onChange={(e) => setAdminNoteInput(e.target.value)}
                                            placeholder="Contoh: Bukti transfer valid, berkas lengkap"
                                            className="w-full bg-slate-950 border border-slate-800 rounded-lg px-3 py-2 text-xs text-slate-200 outline-none focus:border-amber-400"
                                        />
                                    </label>
                                    <div className="flex gap-3 justify-end pt-2">
                                        ...
                                        <button
                                            ...
                                            className="px-5 py-2 rounded-full bg-emerald-400 text-slate-950 font-semibold text-xs hover:scale-105 transition-transform disabled:opacity-50"
                                        >
...
function RegistrationTable({
    ...
    if (rows.length === 0) {
        return <p className="text-slate-500 text-sm py-8 text-center">Belum ada data.</p>
    }
    return (
        <div className="overflow-x-auto">
            <table className="w-full text-left text-xs border-collapse">
                ...
                <tbody className="divide-y divide-slate-800/60">
                    {rows.map((row) => (
                        <tr key={row.id} className="hover:bg-slate-800/30 transition-colors">
                            ...
                            {showKategori && <td className="py-3.5 px-4 font-medium text-emerald-400">{row.kategori || '-'}</td>}
                            <td className="py-3.5 px-4 font-semibold text-white">{row.nama_peserta || row.nama_tim || row.nama || '-'}</td>
                            <td className="py-3.5 px-4 text-slate-400">{row.no_hp || row.whatsapp || '-'}</td>
                            <td className="py-3.5 px-4 text-slate-400">{row.asal_daerah || row.instansi || '-'}</td>

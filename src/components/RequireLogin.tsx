// src/components/RequireLogin.tsx
import { useAuth } from '../context/AuthContext'
import { ReactNode, useState } from 'react'
import NavbarSimple from './NavbarLogo'

interface RequireLoginProps {
    children: ReactNode
}

export default function RequireLogin({ children }: RequireLoginProps) {
    const { user, loading, loginWithGoogle } = useAuth()
    const [isAuthenticating, setIsAuthenticating] = useState(false)
    const [errorMessage, setErrorMessage] = useState<string | null>(null)

    const handleLogin = async () => {
        try {
            setIsAuthenticating(true)
            setErrorMessage(null)
            await loginWithGoogle()
        } catch (error) {
            console.error('Failed to initiate Google OAuth:', error)
            setErrorMessage('Gagal menghubungkan ke Google. Silakan coba lagi.')
            setIsAuthenticating(false)
        }
    }

    // State 1: Memuat status sesi dari Supabase
    if (loading) {
        return (
            <div className="min-h-screen bg-night flex flex-col items-center justify-center text-cream/50 gap-3">
                <div className="w-8 h-8 border-2 border-cream/20 border-t-cream rounded-full animate-spin" />
                <span className="text-sm font-medium">Memuat sesi...</span>
            </div>
        )
    }

    // State 2: User belum terautentikasi
    if (!user) {
        return (

            <div className="min-h-screen bg-night text-cream flex flex-col items-center justify-center px-6 text-center">
                <NavbarSimple />
                <h2 className="font-display text-2xl font-bold mb-3">Login Dulu Yuk</h2>
                <p className="text-cream/60 max-w-sm mb-8">
                    Untuk mendaftar lomba, kamu perlu login dengan akun Google terlebih dahulu.
                </p>

                {errorMessage && (
                    <div className="mb-4 px-4 py-2 rounded-lg bg-red-500/10 border border-red-500/20 text-red-400 text-sm max-w-sm">
                        {errorMessage}
                    </div>
                )}

                <button
                    type="button"
                    onClick={handleLogin}
                    disabled={isAuthenticating}
                    className="flex items-center gap-3 px-6 py-3 rounded-full bg-cream text-night font-semibold hover:scale-105 active:scale-95 transition-all disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100"
                >
                    {isAuthenticating ? (
                        <div className="w-5 h-5 border-2 border-night/20 border-t-night rounded-full animate-spin" />
                    ) : (
                        <svg width="20" height="20" viewBox="0 0 24 24" aria-hidden="true">
                            <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" />
                            <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" />
                            <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" />
                            <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" />
                        </svg>
                    )}
                    <span>{isAuthenticating ? 'Mengarahkan ke Google...' : 'Login dengan Google'}</span>
                </button>
            </div>
        )
    }

    // State 3: User terautentikasi
    return <>{children}</>
}